...
export function CheckForm({
  ...
  // once the draft actually differs from what is stored.
  const dirty = saved
   ...
   */
  const everyKind = appliesTo.length === 0;
  ...
  return (
    <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col gap-4">
      <DialogBody className="space-y-4">
        ...
        <Disclosure label="Scope" summary={describeScope(draft)}>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <label className="flex cursor-pointer items-center gap-2 text-body-sm text-pale-stone">
                <input
                  ...
                  name="check-applies-mode"
                  checked={!everyKind}
                  onChange={() => setAppliesTo([...WORKLOAD_KINDS])}
                  className="accent-warm-off-white"
                />
                Only these
              </label>
        ...
        <Disclosure
          ...
          summary={[
            requires
              ? `needs ${REQUIREMENT_LABEL[requires as keyof typeof REQUIREMENT_LABEL] ?? requires}`
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="check-absent">
                Clean runs before auto-resolving
              </Label>
              <Input
                id="check-absent"
                type="number"
      ...
      <ModalFooter>
        <Button type="submit" disabled={busy || (editing && !dirty)}>
        ...
        {editing && dirty && (
          <span className="text-body-sm text-bone-gray">Unsaved changes</span>
        )}
      </ModalFooter>
    </form>
  );
